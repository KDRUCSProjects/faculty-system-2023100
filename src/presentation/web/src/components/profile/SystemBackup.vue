<template>
  <v-card max-width="450" class="mx-auto d-flex justify-center align-center flex-column pa-5 py-12 theShadow rounded">
    <v-card-title>
      <h2>{{ $t('System Backup') }}</h2>
    </v-card-title>
    <v-card-subtitle>{{ $t('Generate data backup at current state of system') }}</v-card-subtitle>
    <v-card-text>
      <!-- <v-alert
        type="info"
        variant="outlined"
    ...
    <v-card-actions class="mt-8">
      <v-btn @click="startBackup" variant="flat" class="px-6" :loading="isLoading">{{ $t('Start Backup') }}</v-btn>
