...
export default {
  async loadTeachers(context) {
    try {
      ...
      context.commit('setToast', [0, e.response.data.message || context.rootState.$t('All Teacher load failed') ], { root: true });
  ...
  async addTeacher(context, payload) {
    try {
      ...
      context.commit('setToast', context.rootState.$t('Teacher account has been added successfully'), { root: true });
    } catch (e) {
      context.commit('setToast', [0, context.rootState.$t('Failed adding Teacher account') ], { root: true });
      throw e.response.data.message;
    }
  },
  async deleteTeacher(context, teacherId) {
    try {
      ...
      context.commit('setToast', context.rootState.$t('Teacher successfully deleted'), { root: true });
    } catch (e) {
      context.commit('setToast', [0, e.response.data.message || context.rootState.$t('Failed deleting Teacher')], { root: true });
      throw e.response.data.message;
    }
  },
  async updateTeacher(context, payload) {
    try {
      ...
      context.commit('setToast', context.rootState.$t('Teacher data successfully updated'), { root: true });
      // Plus, also update existing teacher data
      // await context.dispatch('loadTeacherById', payload.teacherId);
    } catch (e) {
      context.commit('setToast', [0, e.response.data.message || context.rootState.$t('Failed updating Teacher data') ], { root: true });
      throw e.response.data.message;
    }
  },
